...
def mean_average_precision(pred_boxes, true_boxes, iou_threshold=0.5, box_format='corners', num_classes=20):
    ...
    # box_format : 'corners' or 'midpoint'
    ...
    for c in range(num_classes):
        ...
        # img 0 has 3 bboxes
        # img 1 has 5 bboxes
        # amount__bboxes {dictionary}: {0:3, 1:5}
        ...
        detections.sort(key=lambda x: x[2], reverse=Ture) # x[2]:prob_score
        ...
        for detection_idx, detection in enumerate(detections):
            ...
            if best_iou > iou_threshold:
                if amount_bboxes[detection[0]][best_gt_idx] == 0:
                    TP[detection_idx] = 1 
                    amount_bboxes[detection[0]][best_get_idx] = 1
                else:
                    FP[detection_idx] = 1
        ...
        recalls = torch.cat((torch.tensor([0]), recalls)) # x-axis_start 0
        precisions = torch.cat((torch.tensor([1]), precisions)) # y-axis_start 1
        
        #torch.trapz(y,x) : integral x-y graph
